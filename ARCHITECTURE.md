...
The AI OS is a generic AI‑assistant workspace structured like an operating system. It turns a folder on your machine into a persistent, intelligent business assistant that:

- Knows your business (context files filled by a setup wizard)
- Writes in your voice (voice guide captured during setup)
- Runs structured workflows (14 skills — from research to slide generation)
...
It requires only a compatible AI service subscription. No other API keys are needed for the core 7 starter skills; power skills optionally require external APIs.
...
Someone (Liam Ottley) sells a proprietary "template" — a folder structure with a system prompt — for $997 via a webinar funnel. His framing is actually correct: the runtime is a "harness" (not a wrapper), and structuring it well is the key to getting value. But a folder structure is not worth $997.

Meanwhile, other implementations exist:
- **Obie Fernandez** built a CTO OS — real results (11K lines of institutional knowledge in 3 weeks), but it's all advisory. The model reasons about everything. No deterministic scripts. This works for processing meeting notes but fails at multi-step pipelines where accuracy matters.
...
Our lead research pipeline: 7 scripts, $0.40/lead, 45-60 seconds, consistent results. That's not possible when the model does everything itself.
...
|  skills/  .aios/agents/  plugin.json                |
|  14 skills        3 agents         Distributable bundles      |
|                                                               |
...
|  SYSTEM.md        args/*.yaml      context/                   |
|  .aios/rules/     preferences      voice, business, ICP       |
+--------------------------------------------------------------+
```

| OS Concept | AI OS Equivalent | What It Does |
|------------|-----------------|-------------|
| **Kernel** | SYSTEM.md + .aios/rules/ | System instructions. How the AI thinks and operates. |
| **Programs** | skills/ | Self-contained workflows. Auto-discovered by description matching. |
| **Security** | hooks/ | Lifecycle hooks that fire on events. Block dangerous commands, capture memory, validate output. |
| **Workers** | .aios/agents/ | Specialized subprocesses with restricted tools and cheaper models. |
...
| **Cron** | `aios -p` headless | Schedule skills to run automatically via crontab. |
...
```
aios/
├── SYSTEM.md                                    # KERNEL — System instructions (~80 lines)
...
**Stats:** 53 files, 34 directories, 232K total.
...
**Why:** Self-containment. Each skill is portable, discoverable (the runtime matches descriptions automatically), and can specify its own model, isolation, and tool restrictions via frontmatter.

### 2. SYSTEM.md Is Lean (~80 lines)

The kernel is instructions only. No philosophy, no framework explanation, no "why this structure exists" sections. The model needs to know WHAT to do, not WHY the architecture exists.

Detailed rules go in `.aios/rules/*.md` (modular, loaded automatically). Detailed skill instructions go in each skill's SKILL.md (loaded on demand, not every session).

### 3. Zero API Keys for Core

The 7 starter skills work with ONLY a compatible AI service subscription. No OpenAI key, no Pinecone, no Gmail API. This means:

- `research` uses WebSearch (built into the runtime)
- `email-assistant` works by paste (user pastes email text)
- `task-manager` uses SQLite (standard library)
- `content-writer` uses the model's reasoning (no external calls)
...
Hooks are deterministic safety nets. The model can be convinced to ignore a rule in SYSTEM.md. It cannot bypass a hook that returns exit code 2.
...
**Use MCP when:** the model needs to reason about WHAT to query (exploration).
...
1. Runs `aios` → SYSTEM.md loads as system prompt
4. SYSTEM.md detects `context/my-business.md` is a placeholder
5. Triggers `business-setup` skill automatically
...
1. Runtime starts, loads SYSTEM.md + .aios/rules/ + MEMORY.md
...
4. The runtime matches the request to the `content-writer` skill
...
1. Cron job fires: `aios -p "/weekly-review" --output-format json`
2. The runtime loads headlessly, triggers weekly-review skill
...
- **Description matching** — Does the runtime find the right skill? If not, tweak the description in the SKILL.md frontmatter.
...
Or manually: `python3 skills/skill-creator/scripts/init_skill.py my-skill`
...
Edit `.aios/settings.local.json` — see docs/MCP-SERVERS.md for configs.
...
Edit `.aios/settings.local.json` to add hooks for new lifecycle events. See the settings.local.json.example for the format.
...
| **Liam's $997 template** | Marketing, community | Specifics, battle-testing, open source |
| **Bijit's conceptual OS** | Good framing | Any implementation |
| **Vendor auto‑memory** | Simple persistence | Dedup, semantic search, tiered architecture |
| **Raw AI runtime** | Everything above as primitives | Structure, workflows, business context |

**This system** = structured workspace + separation of concerns + 14 skills + 3 agents + 3-tier memory + hooks + setup wizard + zero-config starter + documented upgrade paths. Built over 6+ months of production use.
...
| Component | Cost |
|-----------|------|
| AI runtime subscription | varies |
...
| **Minimum viable AI OS** | corresponds to whatever your runtime costs |
