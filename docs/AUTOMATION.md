...
Schedule skills to run automatically using your AI runtime's headless mode.
...
```bash
# Run a skill
aios -p "/weekly-review" --output-format json

# Run with custom input
aios -p "/research-lead https://linkedin.com/in/someone" --output-format json

# Run with a natural language prompt
aios -p "Give me my morning briefing" --output-format json
```
...
<dict>
    ...
    <array>
        <string>/usr/local/bin/aios</string>
        <string>-p</string>
...
0 9 * * 1 cd /path/to/ai-os && aios -p "/weekly-review" --output-format json >> data/cron.log 2>&1

# Daily email digest at 7am
0 7 * * * cd /path/to/ai-os && aios -p "/email-digest" --output-format json >> data/cron.log 2>&1

# Morning briefing at 7:30am weekdays
30 7 * * 1-5 cd /path/to/ai-os && aios -p "Give me my morning briefing" --output-format json >> data/cron.log 2>&1
```
...
while IFS= read -r url; do
    aios -p "/research-lead $url" --output-format json >> data/batch-results.json &
done < leads.txt
wait
...
- Test manually first: `aios -p "/skill-name"` before scheduling
