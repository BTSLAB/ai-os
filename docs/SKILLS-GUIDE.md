...
A skill is a self-contained workflow package in `skills/`. Each skill folder contains:
...
Skills are auto-discovered by the runtime based on their description. When you ask the assistant to do something, it checks all skill descriptions to find a match.
...
```bash
python3 skills/skill-creator/scripts/init_skill.py my-skill-name
```
...
| `model` | Which AI model runs it (opus/sonnet/haiku) | Inherits parent |
...
The description is the runtime's only clue about whether to use your skill.
...
- `business-setup` — Conversational wizard (instruction-only)
